---
name: ubi-launch
description: Launch a memecoin on ubi.fun (Arc mainnet) through the ubi MCP server, one transaction, no launch fee. The creator keeps the revenue NFT and 56% of every swap fee.
---

# Launch a coin on ubi.fun

Connect the MCP server first (see the `ubi` skill). Launching is two tool calls plus one signed transaction.
